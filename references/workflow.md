...
19. Produce the versioned verification contract from
...
20. Identify applicable Supabase/Auth/RLS/SSR-cookie/redirect/storage/hosted
...
manifest with `--matrix-output` and the user's `--report-language`. Publish the generated compact Markdown table in
the main task before declaring completion; a ledger artifact without the
tables is insufficient. Include every authoritative, duplicate, failed, or cancelled session
...
16. Produce the completion or checkpoint report for every requested ticket,
    including blocked, failed, cancelled, and completed work. Embed the
    mandatory ticket/transverse-responsibility by phase token table generated by
    `token_usage.py`; do not replace them with aggregate prose.
